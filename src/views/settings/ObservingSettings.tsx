...
export default function ObservingSettings({
  ...
  /** One change, saved and then shown as the file holds it. */
  function update(
  ...
  async function add() {
    try {
      ...
      if (read.state === 'unreadable') {
        says.failure(`Could not add that folder: ${describeUnreadable(read.reason)}`, 'observing-add')
        return
      }

      const listed = observing.repositories.find(
        ({ repository }) => repository === read.repository,
      )
      if (listed !== undefined) {
        says.say(`Already added as ${repositoryName(listed.repository)}.`, 'observing-add')
        return
      }
      ...
      update((current, now) => addRepository(current, added, now), {
        id: 'observing-add',
        saved: `${repositoryName(added.repository)} added. Tick the addresses that are you.`,
        couldNot: 'Could not add that repository.',
      })
    } catch (error) {
      console.error('could not add a repository', error)
      says.failure('Could not add that repository.', 'observing-add')
    }
  }
...
function RepositoryEntry({
  ...
  for (const identity of suggested) {
    if (!offered.some((each) => each.toLowerCase() === identity.toLowerCase())) {
  ...
  function tick(identity: string, ticked: boolean) {
    onIdentities(
      ticked
        ...
        : listed.identities.filter((each) => each !== identity),
    )
  }

  const prefixesId = `observing-prefixes-${name}`
    ...
    <div className="flex flex-col gap-2 rounded-md border border-border p-3">
      ...
      <fieldset className="flex flex-col gap-2 pl-1">
        ...
        {offered.map((identity) => {
          const id = `observing-${name}-${identity}`
          return (
            <div key={identity} className="flex items-center gap-2">
              <Checkbox
                id={id}
                checked={listed.identities.includes(identity)}
      ...
      <Textarea
        ...
        placeholder="Release "
        onChange={(event) => {
          setPrefixes(event.target.value)
          onIgnoredPrefixes(event.target.value.split('\n'))
        }}
      />
    </div>
  )
