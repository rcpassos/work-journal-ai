...
**Tray Menu**:
The menu bar icon's menu. The Entry Point that always works, offering both New Note and New Task so it is the fallback when either configured combination is unavailable. The way into the Main Window as well, and the one Entry Point that names which section to land on — View Notes, View Tasks, Work Summary and Settings each open the Main Window on their section. Work Summary opens the Work Summary section without spending a call. Also the one place the journal is read back without opening a window: Copy Yesterday's Notes puts Yesterday's Digest on the clipboard. And where a Pause is taken and resumed — done in the moment, so it is here rather than only in Settings: while one is in force the offer reads as the pause and its end instead, with Resume Observing under it. Check for Updates… opens Settings, where the check itself lives. The menu is read as it opens, so what it says is true at that moment — a pause that ran out while it was shut reads as ended.
...
**Settings**:
What the user gets to decide about the app: the Note and Task Hotkeys, the Theme, whether the app starts at login, whether today's meetings are imported and from which calendars, whether commits are observed and from which repositories, and how to recover unavailable Task Alert Permission — plus Export, Backup, Update and the Changelog as its actions and answers rather than settings. A Main Window section, reached from the sidebar or named directly from the Tray Menu.
...
**Observing**:
Whether Observe runs over commits, from which repositories, and which identities count as the user in each. Off until turned on, and a repository added through a folder picker — a path is never typed — with its identities offered from what the repository suggests and counted only once ticked; no repository, or no identity ticked, sweeps nothing. A second worktree of a listed repository is the entry it already is and adds nothing. Consent is kept per repository as the intervals it was on — Observing on and the repository listed — and a commit is observed only if it was authored inside one, within the last seven days: filtered by when the work happened, never by when a sweep found it, so work from before enablement, from a gap while it was off, or from while a repository was off the list never arrives. Each repository may skip commits whose subject begins with a prefix the user wrote; a skipped commit is not handled, so removing the prefix lets it arrive within the lookback. Beside each repository the section says what it is doing: why it cannot be read when it cannot be, and the last Note it produced and when — or that nothing has arrived since Observing was turned on. Always Unfiled, and nothing is ever inferred from a path. The repositories are only ever read.
_Avoid_: Git sync, integration, commit import

**Pause**:
A stretch during which work done in the repositories never becomes a Note, taken in the moment from the Tray Menu or from Settings — a screen share, a client's confidential work. An interval and never a flag, kept per source beside consent and pruned with it: pause at 10:00 and resume at 11:00, and a commit authored at 10:30 is refused by every sweep that ever meets it, however late and across any restart, because the filter is on when the work happened. Several pause/resume pairs in one day each exclude their own interval. A pause for an hour or until tomorrow ends by itself — nothing is remembered to undo — and one with no end holds until resumed. While one is in force the Tray Menu and Settings both say so and until when, and a repository added meanwhile joins it: a pause is of Observing, not of one repository.
_Avoid_: Mute, suspend, snooze, do not disturb
